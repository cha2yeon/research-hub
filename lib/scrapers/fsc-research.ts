...
export async function fetchFscResearchReports(): Promise<Report[]> {
  try {
    ...
    const relevantReports = recentReports.filter(isBankingRelevant);
    if (process.env.NODE_ENV === 'development') {
      console.info(
        `[${FSC_ORGANIZATION}] deduplicated=${reports.length} recentTotal=${recentReports.length} filteredTotal=${relevantReports.length}`,
      );
    }
    const displayedReports = relevantReports.length > 0 ? relevantReports : recentReports;
